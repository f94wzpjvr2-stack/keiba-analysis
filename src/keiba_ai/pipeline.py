...
def run_analysis(race, horses, wide_text="", quinella_text="", trio_text="", model_path=None):
    ...
    if not w.empty:
        tables.append(combine_ev(pair_probabilities(scored, "wide"), w, "wide"))
    if not q.empty:
        tables.append(combine_ev(pair_probabilities(scored, "quinella"), q, "quinella"))
    if not t.empty:
        tables.append(combine_ev(trio_probabilities(scored), t, "trio"))
